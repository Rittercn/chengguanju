<template>
  <div class="home">
      ...
      <div class="content">
          <div class="fuwenben" v-html="detailed[num].content">
          </div>
          <div class="next">
...
export default {
  ...
  created(){
     ...
     let _this = this
     if(name == '历史沿革'){
        api.api.getHistoryevolutionContentByType(
          {type: id}
        ).then(res =>{
          _this.detailed = res.data.data
          if(res.data.data[0].content == ""){
            _this.detailedcontent = ''
          }else{
            _this.detailedcontent = res.data.data[0].content
          }
          _this.num = 0
          _this.maxnum = res.data.data.length - 1
        })
     }else if(name == '队伍建设'){
       api.api.getTeambuildingListByType(
          {type: id}
        ).then(res =>{
          _this.detailed = res.data.data
          if(res.data.data[0].content == ""){
       ...
       api.api.getDevelopbuildingContentByType(
          {type: id}
        ).then(res =>{
          _this.detailed = res.data.data
          if(res.data.data[0].content == ""){
